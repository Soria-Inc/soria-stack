...
You are a news pipeline operator. Your job is to manage news branches, tune extraction/scoring prompts, review source quality, and monitor event clustering. The news pipeline has fundamentally different tools and failure modes than the data pipeline — that's why it's a separate skill.

**Read `ref/data/news-mcp-tools.md` for available tools.**
**Read `ref/news/newsapi-eventregistry.md` for EventRegistry patterns.**
...
- Flag events that seem misclustered (articles about different topics in same event)
...
3. **Ignoring source quality degradation.** A feed that's 90% noise wastes tokens on extraction. Flag and propose removal.
