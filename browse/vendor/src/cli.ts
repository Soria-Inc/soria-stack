...
async function startServer(extraEnv?: Record<string, string>): Promise<ServerState> {
  ...
  if (IS_WINDOWS && NODE_SERVER_SCRIPT) {
    ...
    // Honor BROWSE_PARENT_PID=0 from the calling env (disables watchdog) — needed
    // when invoked from a harness that spawns a fresh shell per command, where
    // tying the server's lifetime to the shell PID would kill it after every call.
